...
import 'package:real_estate_app/marketer/marketer_notification_details.dart';
...
class MarketerNotifications extends StatefulWidget {
  final String token;
  ...
  @override
  State<MarketerNotifications> createState() => _MarketerNotificationsState();
}

class _MarketerNotificationsState extends State<MarketerNotifications>
  ...
  Future<void> _fetchCounts() async {
    try {
      final counts = await widget.api.getMarketerUnreadCounts(widget.token);
      setState(() {
  ...
  Future<void> _fetchPage({int page = 1, bool reset = false}) async {
    ...
    try {
      final dynamic resp = await widget.api.fetchMarketerNotifications(
  ...
  Future<void> _markReadInBackground(Map<String, dynamic> item) async {
    ...
    try {
      await widget.api.markMarketerNotificationRead(
  ...
  Future<void> _markUnreadOptimistic(int index) async {
    ...
    _syncHeaderCounts(unreadNotifications: _unread);
    try {
      await widget.api.markMarketerNotificationUnread(
          token: widget.token, userNotificationId: un['id'] as int);
      await _fetchCounts();
    } catch (e) {
      setState(() {
  ...
  Future<void> _markAllRead() async {
    ...
    try {
      final resp =
          await widget.api.markMarketerAllNotificationsRead(token: widget.token);
      setState(() {
  ...
  Future<void> _openDetail(int index) async {
    ...
    try {
      await Navigator.of(context).push(
        MaterialPageRoute(
            builder: (_) => MarketerNotificationDetailPage(
                token: widget.token,
